...
def onehot_object_wise_cross_entropy(
        input: Tensor,
        target: Tensor,
        mask: Tensor,
        length: Tensor,
    ...
    target = F.one_hot(target, 3).float().to(device)
    target[target.sum(dim=1)[:, 2]!=1] = 1 / 3 * torch.ones(target[target.sum(dim=1)[:, 2]!=1].shape).float().to(device)  # 0.33 for all bkg
...
def saja_dilepton_loss(
    ...
    # (QCD, t. tbar)
    permuted_target = target.clone()
    permuted_target[target == 1] = 2
    permuted_target[target == 2] = 1
